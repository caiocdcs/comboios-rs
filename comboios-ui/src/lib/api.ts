...
export async function getStationTimetable(
  ...
  const response = await fetch(
    `/stations/timetable/${encodeURIComponent(stationId)}`,
  );
...
export async function getTrainJourney(
  ...
  let url = `/trains/${encodeURIComponent(trainId)}/journey`;
  if (date) {
  ...
  return {
    ...
    stops: journey.stops.map((stop: any) => {
      ...
      return {
        ...
        station_id:
          typeof stop.station === "string" ? stop.station : stop.station.code,
